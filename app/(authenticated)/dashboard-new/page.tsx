...
export default function DashboardNew() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // Fetch data from the database only once on initial load
  ...
  }, []); // Remove router from dependencies to prevent potential re-fetching

  // Function to fetch data from the database - using the same approach as the preview tables
  const fetchDataFromDatabase = async () => {
    // If data has already been loaded, don't fetch again
    ...
    try {
      ...
      // After successful loading, set the ref to true
      ...
      // Even with error, mark as loaded to prevent auto-retries
      dataLoadedRef.current = true;
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSaveSupplier = async (supplier: Supplier) => {
    ...
    if (supplier.id) {
      ...
      const newSupplier = {
        ...
        id:
          suppliers.length > 0
            ? Math.max(...suppliers.map((s) => s.id || 0)) + 1
            : 1,
        cnCodes:
  ...
  const handleImport = (data: {
    ...
    const lastId =
      suppliers.length > 0 ? Math.max(...suppliers.map((s) => s.id || 0)) : 0;
    ...
    <TooltipProvider>
      <div className="w-full max-w-none px-4 sm:px-6 lg:px-8">
        ...
        <SupplierGoodRecords
          ...
          // Pass import-related props
          importedFiles={importedFiles}
          onImport={handleImport}
          onDownloadImport={handleFileDownload}
          onUndoImport={handleFileUndo}
          onSelectImport={handleFileSelect}
          //useCardWrapper={false}
          //hideCard={true}
          //continuousIndexing={true} // Add this line to enable continuous indexing
        />
