...
export default function Dashboard() {
  ...
  const fetchDataFromDatabase = async () => {
    ...
    try {
      ...
      const fetchSuppliersResult = async () => {
        try {
          ...
          const transformedSuppliers = dbSuppliers.map((supplier) => {
            ...
            const lastUpdateIso = lastUpdateDate ? lastUpdateDate.toISOString() : undefined
      ...
      const fetchGoodsResult = async () => {
        try {
          ...
          const transformedGoods = dbGoods.map((good) => {
            ...
            const goodDate = unixTimestampToDate(good.date ? Number(good.date) : null)
      ...
      if (suppliersResult.success && suppliersResult.data) {
        ...
        console.warn("Using sample supplier data due to fetch error:", suppliersResult.error)
      ...
      if (goodsResult.success && goodsResult.data) {
        ...
        console.warn("Using sample goods data due to fetch error:", goodsResult.error)
  ...
  const filteredSuppliers = useMemo(() => {
    ...
    if (company.name && company.name !== "Example Corp") {
      return suppliers.filter((supplier) => supplier.status !== SupplierStatus.Pending)
    }
  ...
  const handleSaveSupplier = (supplier: Supplier) => {
    ...
    if (supplier.id) {
      const updatedSuppliers = filteredSuppliers.map((s) => (s.id === supplier.id ? updatedSupplier : s))
      setSuppliers(updatedSuppliers)
      sharedSuppliers = updatedSuppliers
    } else {
      const newSupplier = {
        ...updatedSupplier,
        id: filteredSuppliers.length > 0 ? Math.max(...filteredSuppliers.map((s) => s.id || 0)) + 1 : 1,
  ...
  const handleRemoveSuppliers = () => {
    const updatedSuppliers = filteredSuppliers.filter((s) => !selectedSuppliers.includes(s.id!))
    setSuppliers(updatedSuppliers)
    sharedSuppliers = updatedSuppliers
    setSelectedSuppliers([])
  }
  ...
  const handleImport = (data: { suppliers: Supplier[]; goodsImports: GoodsImportRow[]; period: string }) => {
    ...
    const lastId = filteredSuppliers.length > 0 ? Math.max(...filteredSuppliers.map((s) => s.id || 0)) : 0
  ...
  const handleFileUndo = (fileId: string) => {
    ...
    if (file) {
      ...
      const updatedSuppliers = filteredSuppliers.filter((s) => !file.supplierIds.includes(s.id as number))
      setSuppliers(updatedSuppliers)
      sharedSuppliers = updatedSuppliers
  ...
  const handleFileDelete = (fileId: string) => {
    ...
    setSelectedSupplierFiles((prev) => prev.filter((file) => file.id !== fileId))
  ...
  const handleSaveGoodsEntry = (updatedEntry: GoodsImportRow) => {
    setGoodsImports((prevGoodsImports) =>
      prevGoodsImports.map((entry, index) => (selectedGoodsEntries.includes(index) ? updatedEntry : entry)),
    )
  ...
  const handleUpdateSupplier = (updatedSupplier: Supplier) => {
    ...
    const updatedSuppliers = filteredSuppliers.map((s) => (s.id === updatedSupplier.id ? updatedSupplier : s))
    setSuppliers(updatedSuppliers)
    sharedSuppliers = updatedSuppliers
  }
    ...
    <TooltipProvider>
      <div className="max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 mb-8 flex justify-end">
          <Button onClick={() => setIsImportModalOpen(true)} className="bg-black text-white hover:bg-gray-800">
        ...
        {/* Enhanced Dashboard */}
        <EnhancedDashboard suppliers={filteredSuppliers} goodsImports={goodsImports} />
