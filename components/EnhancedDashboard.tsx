...
import { ArrowUpIcon, ArrowDownIcon, Check, FileDown, Clock, Download, CheckCircle2, Users } from "lucide-react"
...
function DonutChart({ data, centerText, centerSubText }: DonutChartProps) {
  ...
  const segments = data.map((item) => {
    ...
    const path = percentage === 0 ? "" : `M ${x1} ${y1} A ${innerRadius} ${innerRadius} 0 ${largeArcFlag} 1 ${x2} ${y2}`
  ...
  if (total === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <div className="text-gray-400 text-sm mt-2">Add suppliers to see statistics</div>
      </div>
    )
  }
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center">
      <svg viewBox="0 0 200 200" className="w-80 h-80">
        {/* Background circle */}
        <circle cx="100" cy="100" r={innerRadius} fill="none" stroke="#f1f5f9" strokeWidth={strokeWidth} />
          ...
          {reportExists ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-lg font-medium">Report Available for {activeQuarter}</span>
                <span className="text-xs text-gray-500">Ready to download and submit</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-lg font-medium">No Report Available for {activeQuarter}</span>
                <span className="text-xs text-gray-500">Complete data collection first</span>
              </div>
            </div>
          )}
...
export function EnhancedDashboard({ suppliers, goodsImports }: EnhancedDashboardProps) {
  ...
  const quartersToDisplay = useMemo(() => {
    ...
    allQuarters.sort((a, b) => {
      ...
      return Number.parseInt(bQ.substring(1)) - Number.parseInt(aQ.substring(1))
    })
  ...
  const [activeQuarter, setActiveQuarter] = useState<string>(quartersToDisplay[0] || "all")
  ...
  const getSuppliersByQuarter = (quarter: string): Supplier[] => {
    ...
    return suppliers.filter((supplier) => supplier.id && supplierIdsInQuarter.has(supplier.id))
  }
  ...
  const getSupplierStatusDistribution = (quarter: string) => {
    ...
    return [
      { label: "Not Contacted", value: statusCounts.notContacted, color: "#94a3b8" },
      { label: "Pending", value: statusCounts.pending, color: "#fbbf24" },
      { label: "Contact Failed", value: statusCounts.failed, color: "#ef4444" },
      { label: "Emission Data", value: statusCounts.emissionData, color: "#10b981" },
      { label: "Supporting Docs", value: statusCounts.supportingDocs, color: "#3b82f6" },
    ]
  }
  ...
  const getQuarterStats = (quarter: string) => {
    ...
    const totalImports = filteredImports.reduce((sum, item) => sum + (item.quantity || 0), 0)
    ...
    const coveredSuppliers = filteredSuppliers.filter(
      (s) =>
        s.status === SupplierStatus.EmissionDataReceived || s.status === SupplierStatus.SupportingDocumentsReceived,
    ).length

    const suppliersCoveredPercentage = totalSuppliers > 0 ? Math.round((coveredSuppliers / totalSuppliers) * 100) : 0
    ...
    if (quarter !== "all" && availableQuarters.length > 1) {
      ...
      if (currentIndex > 0) {
        ...
        previousQuarterStats.totalImports = previousImports.reduce((sum, item) => sum + (item.quantity || 0), 0)
        previousQuarterStats.totalEmissions = previousImports.reduce((sum, item) => {
          const emissionFactor = (item.seeDirect || 0) + (item.seeIndirect || 0)
          return sum + emissionFactor * (item.quantity || 0)
        }, 0)
      }
    ...
    const importChange =
      previousQuarterStats.totalImports > 0
        ? ((totalImports - previousQuarterStats.totalImports) / previousQuarterStats.totalImports) * 100
        : 0

    const emissionChange =
      previousQuarterStats.totalEmissions > 0
        ? ((totalEmissions - previousQuarterStats.totalEmissions) / previousQuarterStats.totalEmissions) * 100
        : 0
    ...
    const reportStatusOptions = [
      { text: "Report is ready for Creation", icon: Check, color: "text-gray-800", reportExists },
      { text: "Report created (download)", icon: FileDown, color: "text-gray-800", reportExists },
      { text: "Report is not ready yet", icon: Clock, color: "text-gray-800", reportExists },
    ]
  ...
  const renderTabStatusIcon = (quarterStats: ReturnType<typeof getQuarterStats>, isActive: boolean) => {
    ...
    return <IconComponent className={`h-4 w-4 mr-2 ${isActive ? "text-white" : iconColor}`} />
  }
              ...
              activeQuarter === quarter && (
                <div key={quarter}>
                  <QuarterDashboard
                    ...
                    isPreviousQuarterAvailable={availableQuarters.indexOf(quarter) > 0}
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-1 flex items-center justify-center">
        <div className="h-[320px] w-full flex items-center justify-center">
          <DonutChart data={chartData} centerText={stats.totalSuppliers.toString()} centerSubText="Suppliers" />
        </div>
      </div>
      <div className="md:col-span-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-lg border shadow-sm">
            <h4 className="text-sm font-medium text-gray-500 mb-2">Total Imports</h4>
            <div className="flex items-end justify-between">
              <div className="text-3xl font-bold">{(stats.totalImports / 1000).toLocaleString()} tons</div>
              {isPreviousQuarterAvailable && (
                <div className="flex flex-col items-end">
                  <div className={`flex items-center ${stats.importChange >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {stats.importChange >= 0 ? (
                      <ArrowUpIcon className="h-4 w-4 mr-1" />
                    ) : (
                      <ArrowDownIcon className="h-4 w-4 mr-1" />
                    )}
                    <span className="font-medium">{Math.abs(stats.importChange).toFixed(1)}%</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-1">from {getPreviousQuarter(quarter)}</p>
                </div>
              )}
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg border shadow-sm">
            <h4 className="text-sm font-medium text-gray-500 mb-2">Total Emissions</h4>
            <div className="flex items-end justify-between">
              <div className="text-3xl font-bold">{(stats.totalEmissions / 1000).toLocaleString()} t CO₂</div>
              {isPreviousQuarterAvailable && (
                <div className="flex flex-col items-end">
                  <div className={`flex items-center ${stats.emissionChange >= 0 ? "text-green-600" : "text-red-600"}`}>
                    {stats.emissionChange >= 0 ? (
                      <ArrowUpIcon className="h-4 w-4 mr-1" />
                    ) : (
                      <ArrowDownIcon className="h-4 w-4 mr-1" />
                    )}
                    <span className="font-medium">{Math.abs(stats.emissionChange).toFixed(1)}%</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-1">from {getPreviousQuarter(quarter)}</p>
                </div>
              )}
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-700">Suppliers Covered</span>
                  <span className="text-xl font-bold">{stats.suppliersCoveredPercentage}%</span>
                </div>
                ...
                <p className="text-xs text-gray-400 mt-1">Suppliers with emission data or supporting documents</p>
              </div>
            </div>
          </div>
        </div>
