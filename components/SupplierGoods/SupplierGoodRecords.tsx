...
import { suppliers as suppliersTable, cnCodes as cnCodesTable, persons as personsTable } from "@/lib/db/schema"
...
interface SupplierGoodRecordsProps {
  ...
  onFilterChange: (filterType: "country" | "status", key: string, value: boolean) => void
...
export function SupplierGoodRecords({
  ...
  const [importModalType, setImportModalType] = useState<"suppliers" | "goods">("suppliers")
  ...
  const [openFilter, setOpenFilter] = useState<"country" | "status" | "quarter" | null>(null)
  ...
  const filteredSuppliers = useMemo(() => {
    if (selectedCountries.length === 0 && selectedStatuses.length === 0 && selectedQuarters.length === 0) {
      return allSuppliers
    }

    return allSuppliers.filter((supplier) => {
      const countryMatch = selectedCountries.length === 0 || selectedCountries.includes(supplier.country)
      const statusMatch = selectedStatuses.length === 0 || selectedStatuses.includes(supplier.status)

      // Quarter filtering - check if supplier has goods in any of the selected quarters
      const quarterMatch =
        selectedQuarters.length === 0 ||
        goodsImports.some((g) => g.supplierId === supplier.id && selectedQuarters.includes(g.quarter))

      return countryMatch && statusMatch && quarterMatch
    })
  }, [allSuppliers, selectedCountries, selectedStatuses, selectedQuarters, goodsImports])
  ...
  const paginatedGoodsImports = useMemo(() => {
    ...
    return filteredGoodsImports.slice(startIndex, startIndex + goodsImportItemsPerPage)
  }, [filteredGoodsImports, goodsImportPage, goodsImportItemsPerPage])

  const handleQuarterFilterChange = (quarter: string) => {
    setActiveQuarter((prevQuarter) => (prevQuarter === quarter ? null : quarter))
  ...
  const fetchSuppliersFromDatabase = useCallback(async () => {
    ...
    try {
      ...
      const transformedSuppliers = dbSuppliers.map((supplier) => {
        // Find contact person
        const contactPerson = persons.find((p) => p.id === supplier.contact_person_id) || null

        // Find CN codes for this supplier
        const supplierCnCodes = allCnCodes.filter((cc) => cc.supplier_id === supplier.id).map((cc) => cc.code)
        ...
        const lastUpdateDate = unixTimestampToDate(supplier.last_update ? Number(supplier.last_update) : null)
        const lastUpdateIso = lastUpdateDate ? lastUpdateDate.toISOString() : new Date().toISOString()

        // Convert Unix timestamp to Date object for validUntil
        const validUntilDate = unixTimestampToDate(
          supplier.emission_data_valid_until ? Number(supplier.emission_data_valid_until) : null,
        )
        const validUntilIso = validUntilDate ? validUntilDate.toISOString() : undefined
        ...
        return {
          ...
          // Add SEE values if they exist in the database
      ...
      const processedSuppliers = transformedSuppliers.map((supplier) => {
        if (
          ...
          const baseDate = supplier.lastUpdate ? new Date(supplier.lastUpdate) : new Date()
      ...
      setError(`Failed to fetch suppliers: ${error instanceof Error ? error.message : String(error)}`)
  ...
  useEffect(() => {
    ...
    if (suppliers.length > 0 && JSON.stringify(suppliers) !== JSON.stringify(allSuppliers)) {
      const updatedSuppliers = suppliers.map((supplier) => {
        // Only process suppliers with Emission Data or Supporting Docs status
        if (
          (supplier.status === SupplierStatus.EmissionDataReceived ||
  ...
  const generateExportData = () => {
    ...
    return suppliersToExport.map((supplier) => {
      ...
      const supplierGoods = goodsImports.filter((g) => g.supplierId === supplier.id)
      ...
      return {
        ...
        "See Direct": supplier.seeDirect || 0,
        "See Indirect": supplier.seeIndirect || 0,
        "See Total": supplier.seeTotal || (supplier.seeDirect || 0) + (supplier.seeIndirect || 0),
        "Emission Factor": supplier.emissionFactor || 0,
        "Emissions from electricity usage": supplier.electricityEmissions || 0,
      }
    })
  }
  ...
  const getSuppliersToExport = () => {
    let filteredSuppliers = allSuppliers.filter((supplier) => supplier.status === exportStatus)

    // If not including expired data, filter out expired suppliers
    if (!includeExpiredData) {
      const now = new Date().toISOString()
      filteredSuppliers = filteredSuppliers.filter((supplier) => !supplier.validUntil || supplier.validUntil > now)
    }
      ...
      <Card>
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 py-4">
          <div className="flex flex-col">
            <CardTitle className="text-xl font-semibold my-0">Suppliers</CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <Button onClick={onAddSupplier} className="bg-black text-white hover:bg-gray-800">
              <Plus className="mr-2 h-4 w-4" />
              Add Supplier
            </Button>
            <Button variant="outline" onClick={() => openImportModal("suppliers")} className="flex items-center gap-2">
          ...
          <div className="border-b bg-neutral-50">
            <div className="px-4 py-4">
              <div className="flex flex-col gap-4">
                ...
                {(selectedCountries.length > 0 || selectedStatuses.length > 0 || selectedQuarters.length > 0) && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="relative" ref={countryFilterRef}>
                    ...
                    >
                      ...
                      <span className="text-gray-500">{openFilter === "country" ? "▲" : "▼"}</span>
                    </button>
                      ...
                      <div className="absolute z-10 mt-1 w-full bg-white border rounded-md shadow-lg max-h-60 overflow-auto">
                        <div className="p-2">
                          ...
                          {uniqueCountries.map((country) => {
                            const count = allSuppliers.filter((s) => s.country === country).length
                            return (
                  ...
                  <div className="relative" ref={statusFilterRef}>
                    <button
                      ...
                      className="w-full flex items-center justify-between bg-white border rounded px-3 py-2 text-sm"
                    >
                      <span>Statuses {selectedStatuses.length > 0 && `(${selectedStatuses.length})`}</span>
                      <span className="text-gray-500">{openFilter === "status" ? "▲" : "▼"}</span>
                    </button>
                      ...
                      <div className="absolute z-10 mt-1 w-full bg-white border rounded-md shadow-lg max-h-60 overflow-auto">
                        <div className="p-2">
                          <div className="mb-2 border-b pb-1">
                            ...
                            />
                            <label htmlFor="select-all-statuses" className="ml-2 text-sm font-medium">
                              Select All
                            </label>
                          </div>

                          {uniqueStatuses.map((status) => {
                            const count = allSuppliers.filter((s) => s.status === status).length
                            return (
                              <div key={`status-${status}`} className="flex items-center mb-1">
                                <Checkbox
                                  id={`status-${status}`}
                  ...
                  <div className="relative" ref={quarterFilterRef}>
                    <button
                      onClick={() => setOpenFilter(openFilter === "quarter" ? null : "quarter")}
                      className="w-full flex items-center justify-between bg-white border rounded px-3 py-2 text-sm"
                    >
                      <span>Quarters {selectedQuarters.length > 0 && `(${selectedQuarters.length})`}</span>
                      <span className="text-gray-500">{openFilter === "quarter" ? "▲" : "▼"}</span>
                    </button>
                        ...
                        <div className="p-2">
                          <div className="mb-2 border-b pb-1">
                            <Checkbox
                              ...
                              checked={selectedQuarters.length === availableQuarters.length}
                              onCheckedChange={(checked) => {
                                if (checked) {
                            ...
                            />
                            <label htmlFor="select-all-quarters" className="ml-2 text-sm font-medium">
                              Select All
                            </label>
                          </div>
                          ...
                          {availableQuarters.map((quarter) => {
                            ...
                            const suppliersInQuarter = new Set(
                              goodsImports.filter((g) => g.quarter === quarter).map((g) => g.supplierId),
                            )
                            const count = suppliersInQuarter.size

                            return (
                              <div key={`quarter-${quarter}`} className="flex items-center mb-1">
                                <Checkbox
                                  id={`quarter-${quarter}`}
                                ...
                                />
                                <label htmlFor={`quarter-${quarter}`} className="ml-2 text-sm">
                                  {quarter} ({count})
                                </label>
                              </div>
                            )
        ...
        <CardFooter className="grid grid-cols-3 border-t py-4">
          <div className="flex items-center col-start-1">
            {selectedSuppliers.length > 0 && (
              <Button variant="destructive" onClick={onRemoveSuppliers} className="mr-2">
                Remove Selected
              </Button>
            )}
            <Button variant="outline" onClick={onSendMail} disabled={selectedSuppliers.length === 0}>
          ...
          <div className="flex justify-center col-start-2">
            <PaginationControls
              ...
              totalPages={Math.ceil(sortedSuppliers.length / supplierItemsPerPage)}
      ...
      <Dialog open={isExportModalOpen} onOpenChange={setIsExportModalOpen}>
        <DialogContent className="sm:max-w-[700px] p-0">
          <div className="flex flex-row items-center justify-between space-y-0 py-4 px-6 border-b">
            <DialogTitle className="text-xl font-semibold my-0">Export Supplier Data</DialogTitle>
          </div>

          <div className="px-6 py-4">
            <div className="mb-6">
              <h4 className="text-lg font-medium mb-4">Export suppliers with Emissions Data Received status</h4>
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-md text-blue-700">
                <p>
                  This will export all suppliers with the status <strong>Emissions Data Received</strong>. Currently
                  there are{" "}
                  <strong>{allSuppliers.filter((s) => s.status === SupplierStatus.EmissionDataReceived).length}</strong>{" "}
            ...
            <div className="bg-gray-50 rounded-lg p-4 mt-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Use latest available data, even when expired</p>
                </div>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t">
            ...
            <Button onClick={() => handleExport("xlsx")} disabled={getSuppliersToExport().length === 0}>
              Export as Excel
            </Button>
          </div>
        </DialogContent>
