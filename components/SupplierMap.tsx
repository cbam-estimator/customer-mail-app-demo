...
export default function SupplierMap({
  ...
  const [suppliersWithLocations, setSuppliersWithLocations] = useState<
    SupplierWithLocation[]
  >([]);
  const [selectedSupplier, setSelectedSupplier] =
    useState<SupplierWithLocation | null>(null);
  ...
  useEffect(() => {
    const generatedSuppliers: SupplierWithLocation[] = suppliers.map(
      (name, index) => {
        ...
        if (citiesPerCountry[country] && citiesPerCountry[country].length > 0) {
          const cityData =
            citiesPerCountry[country][
              Math.floor(Math.random() * citiesPerCountry[country].length)
            ];
          city = cityData.name;
        ...
        const supplierImports = goodsImports.filter(
          (item) => item.manufacturer === name
        );
        const totalImports = supplierImports.reduce(
  ...
  const calculateAverages = (supplier: SupplierWithLocation) => {
    ...
    const countrySuppliers = suppliersWithLocations.filter(
      (s) => s.country === supplier.country
    );
    const nationalAvgEmissions =
      countrySuppliers.reduce((sum, s) => sum + (s.totalEmissions || 0), 0) /
      countrySuppliers.length;

    // Global average (all suppliers)
    const globalAvgEmissions =
      suppliersWithLocations.reduce(
        (sum, s) => sum + (s.totalEmissions || 0),
        0
      ) / suppliersWithLocations.length;
