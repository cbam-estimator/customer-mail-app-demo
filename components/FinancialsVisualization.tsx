...
export function FinancialsVisualization({ goodsImports }: FinancialsVisualizationProps) {
  ...
  const [viewMode, setViewMode] = useState<"quarters" | "suppliers">("quarters")
  ...
  useEffect(() => {
    ...
    try {
      ...
      const filteredData = goodsImports.filter((item) => {
        const supplierMatch = selectedSuppliers.includes(item.manufacturer || "Unknown")
        const quarterMatch = selectedQuarters.includes(item.quarter || "Unknown")
      ...
      if (viewMode === "quarters") {
        ...
        detailedData = activeQuarters.map((quarter, index) => {
          ...
          return {
            ...
            costPerTon: importQuantity > 0 ? Math.round((totalCosts / importQuantity) * 100) / 100 : 0,
          }
        })
        ...
        {isFilterOpen && (
          <div className="mb-6 border rounded-lg shadow-sm bg-white overflow-hidden">
            <div className="p-4 bg-gray-50 border-b">
              ...
              <p className="text-sm text-gray-500">Refine your data visualization by applying multiple filters</p>
            </div>
                  ...
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex justify-between items-center">
                      ...
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="xs" onClick={handleClearSuppliers} className="text-xs h-6 px-2">
                          None
                        </Button>
                      </div>
                    </CardTitle>
                  ...
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex justify-between items-center">
                      ...
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="xs" onClick={handleClearQuarters} className="text-xs h-6 px-2">
                          None
                        </Button>
                      </div>
                    </CardTitle>
                        ...
                        <div key={quarter} className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              onCheckedChange={() => handleToggleQuarter(quarter)}
                            />
                            <label
              ...
              <Card className="h-full">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex justify-between items-center">
                    <span>CN Codes</span>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="xs" onClick={handleSelectAllCnCodes} className="text-xs h-6 px-2">
                        All
                      </Button>
                      <Button variant="ghost" size="xs" onClick={handleClearCnCodes} className="text-xs h-6 px-2">
                        None
                      </Button>
                    </div>
                  </CardTitle>
              ...
              <Card className="h-full">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex justify-between items-center">
                    <span>Countries</span>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="xs" onClick={handleSelectAllCountries} className="text-xs h-6 px-2">
                        All
                      </Button>
                      <Button variant="ghost" size="xs" onClick={handleClearCountries} className="text-xs h-6 px-2">
                        None
                      </Button>
                    </div>
                  </CardTitle>
                      ...
                      <div key={category} className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={() => handleToggleGoodCategory(category)}
                          />
                          <label
              ...
              <div className="flex flex-wrap gap-2">
                {selectedSuppliers.length < uniqueSuppliers.length && (
                  <Badge variant="secondary" className="px-2 py-1">
                    Suppliers: {selectedSuppliers.length}/{uniqueSuppliers.length}
                  </Badge>
                )}
                ...
                {selectedCountries.length < uniqueCountries.length && (
                  <Badge variant="secondary" className="px-2 py-1">
                    Countries: {selectedCountries.length}/{uniqueCountries.length}
                  </Badge>
                )}
                {selectedGoodCategories.length < uniqueGoodCategories.length && (
                  <Badge variant="secondary" className="px-2 py-1">
                    Categories: {selectedGoodCategories.length}/{uniqueGoodCategories.length}
                  </Badge>
                )}
              </div>
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {chartData.detailedData && chartData.detailedData.length > 0 ? (
                  chartData.detailedData.map((item: any) => (
                    <tr key={item.label}>
                      <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900">{item.label}</td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">
                        {viewMode === "quarters" ? item.supplierCount : item.quarterCount}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{item.goodsCount}</td>
                      <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-500">
