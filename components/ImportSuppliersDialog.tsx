...
const mapHerstellerColumns = (
  ...
  if (missingColumns.length > 0) {
    throw new Error(
      `Missing required columns in Hersteller: ${missingColumns.join(", ")}`
    );
  }
...
const mapWarenimporteColumns = (
  ...
  if (missingColumns.length > 0) {
    throw new Error(
      `Missing required columns in Warenimporte: ${missingColumns.join(", ")}`
    );
  }
...
const processHerstellerSheet = (sheet: XLSX.WorkSheet): Supplier[] => {
  ...
  for (let R = headerRow + 1; R <= range.e.r; ++R) {
    const nameCell =
      sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Name"] })];
    ...
    const supplier: Supplier = {
      ...
      name: nameCell.v,
      country:
        sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Land"] })]?.v ||
        "",
      address: {
        country:
          sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Land"] })]
            ?.v || "",
        street:
          sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Straße"] })]
            ?.v || "",
        streetNumber:
          sheet[
            XLSX.utils.encode_cell({ r: R, c: columnMapping["Hausnummer"] })
          ]?.v || "",
        additionalLine:
          sheet[
            XLSX.utils.encode_cell({ r: R, c: columnMapping["Adresszusatz"] })
          ]?.v || "",
        postcode:
          sheet[
            XLSX.utils.encode_cell({ r: R, c: columnMapping["Postleitzahl"] })
          ]?.v || "",
        city:
          sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Stadt"] })]
            ?.v || "",
      },
      contactPerson: {
        name:
          sheet[
            XLSX.utils.encode_cell({
              r: R,
              c: columnMapping["Ansprechpartner"],
            })
          ]?.v || "",
        email:
          sheet[
            XLSX.utils.encode_cell({ r: R, c: columnMapping["Email-Adresse"] })
          ]?.v || "",
        phone:
          sheet[
            XLSX.utils.encode_cell({ r: R, c: columnMapping["Telefonnummer"] })
          ]?.v || "",
      },
      cnCodes: [],
      remarks:
        sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Anmerkungen"] })]
          ?.v || "",
...
const processWarenimporteSheet = (
  sheet: XLSX.WorkSheet,
  period: string
): GoodsImportRow[] => {
  const headerRow = findHeaderRow(sheet);
  ...
  const date = new Date(
    ...
    (Number.parseInt(quarter.slice(1)) - 1) * 3,
    1
  );

  for (let R = headerRow + 1; R <= range.e.r; ++R) {
    const cnCodeCell =
      sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["CN-Code"] })];
    ...
    const goodsImport: GoodsImportRow = {
      ...
      remarks:
        sheet[
          XLSX.utils.encode_cell({ r: R, c: columnMapping["Ihre Anmerkungen"] })
        ]?.v || "",
      cnCode: cnCodeCell.v,
      manufacturer:
        sheet[
          XLSX.utils.encode_cell({
            r: R,
            c: columnMapping["Hersteller / Händler / Installation"],
          })
        ]?.v || "",
      quantity:
        sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Warenmenge"] })]
          ?.v || 0,
      unit:
        sheet[XLSX.utils.encode_cell({ r: R, c: columnMapping["Einheit"] })]
          ?.v || "",
      productionMethod:
        sheet[
          XLSX.utils.encode_cell({
            r: R,
            c: columnMapping["Produktionsmethode"],
          })
        ]?.v || "",
      customsProcedure:
        sheet[
          XLSX.utils.encode_cell({ r: R, c: columnMapping["Zollverfahren"] })
        ]?.v || "",
      date: date,
      quarter: period,
...
export function ImportSuppliersDialog({
  ...
  const processExcelFile = async (file: File) => {
    try {
      ...
      if (
        !workbook.SheetNames.includes("Hersteller") ||
        !workbook.SheetNames.includes("Warenimporte")
      ) {
        throw new Error(
          "Excel file must contain both 'Hersteller' and 'Warenimporte' sheets"
        );
      }
      ...
      if (workbook.SheetNames.includes("Allgemeine Informationen")) {
        const companyInfo = processCompanyInfo(
          workbook.Sheets["Allgemeine Informationen"]
        );
        if (Object.keys(companyInfo).length > 0) {
          ...
          toast({
            ...
            description:
              "Company details have been updated from the imported file.",
          });
        }

        // Extract period information
        const allgemeineInformationenSheet =
          workbook.Sheets["Allgemeine Informationen"];
      ...
      const goodsImports = processWarenimporteSheet(
        workbook.Sheets["Warenimporte"],
        period
      );
